...
function getLastCarInfo(inventory) {
  const arrLength = inventory.length - 1;
  return `This is a ${inventory[arrLength].car_make} ${inventory[arrLength].car_model}`;
}
...
function getModelYears(inventory) {
  ...
  for (let index in inventory) {
    carYearArr.push(inventory[index].car_year);
  }
  return carYearArr;
}
...
function carMaker(num) {
  return {
    ...
    drive: function(distance) {
      let odometer = this.odometer + distance;
